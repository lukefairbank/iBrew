...
class iBrewDomoticz:
    ...
    def setup(self,client,connection,prefix="",name=""):
        ...
        if client.isCoffee:
            ...
            id = self.get_id('GRIND','Grind',self.SensorSwitch)
            self.set_switch_type(id,'Grind',self.SwitchTypeMotion,self.CustomImageNormal)
            client.triggerAdd("Domoticz","GRIND",url_trigger_user % (self.domoticzurl,id))
            id = self.get_id('DEFAULTGRIND','Default Grind',self.SensorSwitch)
            self.set_switch_type(id,'Default Grind',self.SwitchTypeMotion,self.CustomImageNormal)
            client.triggerAdd("Domoticz","DEFAULTGRIND",url_trigger_user % (self.domoticzurl,id))
            id = self.get_id('MODE','Mode',self.SensorSwitch)
            self.set_switch_type(id,'Mode',self.SwitchTypeMotion,self.CustomImageNormal)
